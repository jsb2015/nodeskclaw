...
Core Protocol 不强制定义统一 request/response schema，也不要求目标平台为了接入 Agent Device 迁移到固定 wire protocol（传输协议）。v1 后续需要补齐的是 Agent-readable semantic contract（Agent 可理解语义契约）：操作意图、结果语义、状态变化、失败原因、外部副作用、审计证据和能力协商。目标平台可以通过 CLI、Tool、Skill、MCP、HTTP API、SDK 或其他入口实现这些语义，不要求按固定请求/响应结构做协议转换。
...
### 2.3 Level 2 Control Transfer Profile
...
- Core Protocol 的 Agent-readable semantic contract、事件语义和能力协商。
- 通用 Preflight 字段。
