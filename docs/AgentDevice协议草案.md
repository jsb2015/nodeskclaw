...
| 失败原因 | MUST | 失败或拒绝时，说明 Agent 可采取行动的原因，见 2.3 |
...
| 安全重试 | MUST | 说明 Agent 是否可以安全重试，或必须先刷新状态、重新申请租约、请求授权、请求介入 |
| 失败类别 | MUST | 说明失败大致属于权限/准入、租约/占用、状态冲突、能力不支持、风险策略、外部系统失败、审计或证据不可确认、副作用状态未知 |
...
| 是否定义统一错误模型 | 不定义统一错误码或错误返回结构，也不作为后续版本方向。通用协议层只要求最小失败语义，确保 Agent 能判断执行状态、副作用状态、安全重试、失败类别和审计可靠性；下一步动作只在可判断时 SHOULD 提供 |
